...
# Add your SSH public key (ensure no extra whitespace or extraneous text)
...
# - Set a high log level (for debugging; you may reduce later)
# - Allow TTY allocation and restrict login to our user
# - Force a login shell so that an interactive bash session is always started
...
# Adjust PAM so that pam_loginuid is optional (prevents immediate session termination)
RUN sed -i 's/^session\s\+required\s\+pam_loginuid.so/session optional pam_loginuid.so/' /etc/pam.d/sshd

# Copy MongoDB configuration file (suppressing logs, etc.)
