...
# Create the Render service user with a dedicated home directory (ensure it's not on the mounted MongoDB data volume)
...
# Add your SSH public key (ensure the key line is exact; remove any extra spaces or text)
...
# - Increase log level for debugging (you can lower this later)
...
# Copy MongoDB configuration file (this one suppresses logs)
